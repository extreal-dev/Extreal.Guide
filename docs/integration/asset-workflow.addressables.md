...
- OnDownloaded
    ...
    - タイプ：IObservable
    - パラメータ：ダウンロード状況
      - アセット名、トータルバイト数、ダウロード済みバイト数、パーセントなど
- OnConnectRetrying
    - タイミング：接続をリトライする直前
    - タイプ：IObservable
    - パラメータ：リトライ回数
...
- BuildScriptEncryptModeはビルド実行時にAsset Groupに指定されているResourceProviderがCryptoAssetBundleProviderBaseの場合はICryptoStreamFactoryを使って暗号化を行い、そうでない場合は暗号化を行いません。
